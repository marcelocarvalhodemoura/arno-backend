...
async function passwordHashOf(username: string): Promise<string> {
  const result = await getPool().query<{ password_hash: string }>("SELECT password_hash FROM users WHERE username = $1", [
    username,
  ]);
...
async function ensureType(auth: { Authorization: string }, name: string, direction: "income" | "expense" | "both") {
  ...
  const created = await request(server).post("/api/movement-types").set(auth).send({ name, direction, description: name });
...
describe("API integration", () => {
  ...
  it("lets tesoureiro split a credit and read settings", async () => {
    ...
    const split = await request(server)
      ...
      .send({
        parts: [
          { amount: 60, movementTypeId: donation.id, description: "Doação da parte 1" },
          { amount: 90, movementTypeId: camp.id, description: "Acampamento da parte 2" },
        ],
      });
...
`,
  ...
  it("builds the March-December mensalidade grid and launches pending cash-flow rows", async () => {
    ...
    const created = await request(server)
      ...
      .send({
        ...
        joinedAt: "2026-05-10",
        clubeLtc: false,
        guardians: [{ name: "Responsável Mensalidade", relationship: "Mãe", phone: "(51) 99999-3334" }],
      });
    expect(created.status).toBe(201);
  ...
  it("lets the admin read dashboard, users and projects", async () => {
    ...
    const withEmail = await request(server)
      .post("/api/auth/login")
      .send({ user: String(created.body.email).toUpperCase(), password: "novasenha" });
  ...
  it("syncs mock Sicredi Pix into cash flow and is idempotent", async () => {
    ...
    const webhook = await request(server).post("/api/integrations/sicredi/webhook?token=teste-webhook").send({ pix: [] });
  ...
  it("enqueues bulk mensalidade charges and the outbox worker delivers them", async () => {
    ...
    const notified = await request(server)
      ...
      .set(auth)
      .send({ year: 2026, month: 9, kind: "charge", transactionIds: [cell.transactionId] });
