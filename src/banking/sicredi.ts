...
export function sicrediStatus(): SicrediStatus {
  ...
  if (config.mock) {
    return { configured: true, mock: true, environment: "mock", webhookReady: Boolean(config.webhookToken) };
  }
  if (!config.live) {
    return { configured: false, mock: false, environment: "off", webhookReady: false };
  }
...
export async function registerPixWebhook() {
  const config = sicrediConfig();
  if (config.mock) {
    return { webhookUrl: config.publicUrl ? `${config.publicUrl}/api/integrations/sicredi/webhook` : "" };
  }
  if (!config.live) throw new Error("API Pix do Sicredi ainda não está configurada");
  ...
  await sicrediPut(`/webhook/${encodeURIComponent(config.pixKey)}`, { webhookUrl });
  return { webhookUrl };
}
