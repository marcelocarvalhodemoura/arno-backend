...
function emptyDb(): DatabaseShape {
  return {
    ...
    fees: [{ id: "f1", name: "Mensalidade", amount: 55, origin: "manual", createdAt: "2026-01-01T00:00:00.000Z" }],
    transactions: [
      {
        id: "tx-pending",
        date: "2026-09-01",
...
describe("ingestTransactions", () => {
  ...
  it("does not duplicate the same launch in one import or a second file", () => {
    ...
    const first = ingestTransactions(db, [row, { ...row, description: "pix recebido helena souza mensalidade" }], "user-1");
